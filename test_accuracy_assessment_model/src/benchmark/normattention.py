...
normalize = importlib.import_module(".NormAttention-PSN.datasets.pms_transforms", __package__).normalize
NormAttentionPSN = importlib.import_module(".NormAttention-PSN.models.NormAttentionPSN_run", __package__).NormAttentionPSN
net = NormAttentionPSN(c_in=6).to(device)
checkpoint_path = os.path.join("NormAttention-PSN", "data", "models", "NormAttention-PSN-test.pth.tar")
...
def benchmark(images, light_dirs, object_mask, normal_gt=None, dtype=np.float32, debug=False):
  ...
  with torch.no_grad():
    ...
    normal_pred1 = net([images, light_dirs])[0, ...].permute(1, 2, 0).cpu().numpy()
  del images
  del light_dirs
