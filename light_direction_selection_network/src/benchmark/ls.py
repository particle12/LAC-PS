...
def benchmark(images, light_dirs, object_mask, normal_gt=None, dtype=np.float32, debug=False):
  images = images.astype(dtype).copy().mean(axis=-1).transpose(1, 2, 0) #(128, 128, 2）2表示图片数  [0,1]
  light_dirs = light_dirs.astype(dtype).copy()  #(2, 3)
  ...
  intensity_low = np.quantile(images[object_mask, :], dtype(0.0)).astype(dtype) #0.003070154
  intensity_high = np.quantile(images[object_mask, :], dtype(1.0)).astype(dtype) #1.0
  if debug:
    print("intensity_low", intensity_low, "intensity_high", intensity_high)
  images = np.append(images, np.ones([images.shape[0], images.shape[1], 1], dtype=dtype), axis=2) #(128, 128, 3)
  light_num = light_dirs.shape[0] + 1  #3
  light_dirs = np.append(light_dirs, [np.array([0.0, 0.0, 1.0], dtype=dtype)], axis=0) #(3, 3)
  light_dirs = light_dirs[np.newaxis, :, :] #(1, 3, 3)
  light_dirs /= np.linalg.norm(light_dirs, ord=2, axis=2, keepdims=True)
  I = images[object_mask, :, np.newaxis] #(12601, 3, 1)
  weight = np.maximum(0.0, np.minimum(I - intensity_low, intensity_high - I)) #(12601, 3, 1)
  weight[:, -1, :] = 1e-3
  L = np.broadcast_to(light_dirs, [object_mask.sum(), light_num, 3]) * weight #(12601, 3, 3)
  I = I * weight #(12601, 3, 1)
  LTI = np.matmul(L.transpose(0, 2, 1), I) #(12601, 3, 1)
  LTL = np.matmul(L.transpose(0, 2, 1), L) + 1e-6 * np.eye(3, dtype=dtype)[None, ...] #(12601, 3, 3)
  inv_LTL = np.linalg.inv(LTL) #(12601, 3, 3)
  inv_LTL_LTI = np.matmul(inv_LTL, LTI) #(12601, 3, 1)
  N = np.zeros([images.shape[0], images.shape[1], 3], dtype=dtype) #(128, 128, 3)
  N[object_mask, :] = inv_LTL_LTI[..., 0] #(128, 128, 3)
  N /= (np.linalg.norm(N, ord=2, axis=2, keepdims=True) + 1e-6) #(128, 128, 3)
  N[~object_mask, :] = 0.0  #(128, 128, 3)
  normal_gt /= (np.linalg.norm(normal_gt, ord=2, axis=2, keepdims=True) + 1e-6)  #(128, 128, 3)
