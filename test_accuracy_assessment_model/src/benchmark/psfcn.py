...
def benchmark(images, light_dirs, object_mask, normal_gt=None, dtype=np.float32, debug=False):
  images = images.astype(dtype).copy()[:, :, :, ::-1] #bgr_rgb
  # images = images.astype(dtype).copy()[:, :, :, ::-1] #bgr_rgb
  ...
  images = images.transpose(0, 1, 3, 2)[:, :, ::-1, :] #bgr_rgb
  ...
  with torch.no_grad():
    ...
    # normal_pred2 = net([images, light_dirs])
    # normal_pred1 = net([images, light_dirs])[0, ...].permute(1, 2, 0).cpu().numpy()
    # cv2.imwrite(os.path.join("pre_normal1111.png"),(normal_pred1* 0.5 + 0.5).squeeze()*255.0)
    # vutils.save_image(normal_pred2*0.5+0.5, os.path.join("normal_pred2.png"))
...
#   torch.cuda.empty_cache()
  ...
  # normal_pred1 =   normal_pred.deepcopy()
  normal_pred = normal_pred.transpose(1, 0, 2)[:, ::-1, :]
  # normal_pred1 = normal_pred1.transpose(1, 0, 2)
