...
class builder():
    ...
    def run(self, 
        ...
        for batch_test in test_data_loader:                      
            I, N, M, nImgArray, roi = self.separate_batch(batch_test)  #  roi  tensor([[512, 612, 185, 342, 229, 386]])
            roi = roi[0].numpy()  #array([512, 612, 185, 342, 229, 386])
            h_ = roi[0]
            w_ = roi[1]
            r_s = roi[2]
            r_e = roi[3]
            c_s = roi[4]
            c_e = roi[5]
            B, C, H, W, Nimg = I.shape #torch.Size([1, 3, 512, 512, 96])

            """Scalable Reconstruction"""   
            with torch.no_grad():       
                if self.args.scalable:    #false
                    patch_size = 512               
