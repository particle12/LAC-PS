...
def mesh_action_ids(all_light_dirs, mesh_size, dtype=np.float32): #all_light_dirs (100, 3)  mesh_size 12
  mesh_x = np.linspace(-1.0, 1.0, num=mesh_size, endpoint=True, dtype=dtype) #(12,)
  mesh_y = np.linspace(-1.0, 1.0, num=mesh_size, endpoint=True, dtype=dtype) #(12,)
  mesh_x, mesh_y = np.meshgrid(mesh_x, mesh_y, indexing="ij") #(12, 12) (12, 12)
  meshgrid = np.stack([mesh_x.flatten(), mesh_y.flatten()], axis=1) #(144, 2)
  dist = np.linalg.norm(all_light_dirs[:, None, :2] - meshgrid[None, :, :], ord=2, axis=2) #(100, 144)
  ids = dist.argmin(axis=1) #(100,)
  return ids

class Env:
  def __init__(self, input_files, rng, algo, down_sample=2):
    self.input_files = input_files
    self.down_sample = down_sample #2
    self.rng = rng
    self.algo = algo
    self.images = []
    self.normal = []
  ...
  def benchmark(self):
    ...
    return error, error_mean, normal  #(128, 128) #138.23572 #(128, 128, 3)
  ...
  def state(self, dtype=np.float32):
    images = self.images.astype(dtype).transpose(0, 3, 1, 2) #(1, 3, 128, 128)
  ...
  def reset(self, scene_id=None):
    if scene_id is None:
      self.input_file = self.rng.choice(self.input_files)  #'data/train_sculpture/000350.npz'
    else:
      self.input_file = self.input_files[scene_id]
    self.data_loader = np.load(self.input_file, allow_pickle=True)
    self.all_light_dirs = light_dirs(self.data_loader["meta_info"].item()["light_dirs_mode"]) #(100, 3)
    self.ids = [np.argmax(self.all_light_dirs[:, 2])]  #[0]
    self.normal_gt, self.images, _ = imload_downsample_from_ids(self.data_loader, self.ids, self.down_sample) #(128, 128, 3)  (1, 128, 128, 3)
    self.error, _, self.normal_pred = self.benchmark() #(128, 128)  138.23572 (128, 128, 3)
    ...
    return self.state(), self.error #(1, 3, 128, 128)

  def step(self, action):
    self.ids.append(action)
    _, self.images, _ = imload_downsample_from_ids(self.data_loader, self.ids, self.down_sample)
    new_error, error_mean, self.normal_pred = self.benchmark()  #(128, 128) #138.23572 #(128, 128, 3)
