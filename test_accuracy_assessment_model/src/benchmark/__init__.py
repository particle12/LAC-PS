...
def imload_downsample_from_ids_unips_100(data_loader, ids, down_sample=2):
  global _image_cache
  if _image_cache is None:
    _image_cache = {}
    _image_cache_clean_thread = threading.Thread(target=_image_cache_clean, daemon=True)
  ...
  for curr_id, curr_light_dir in zip(ids, selected_light_dirs):
    ...
    if curr_image_key in _image_cache:
      ...
      if os.environ.get("RLPS_ABLATION") == "1":
        ...
        import cv2
        h = w =512
        curr_image =cv2.resize(curr_image, dsize=(h, w),interpolation=cv2.INTER_CUBIC)#(512, 512)
  ...
  normal = cv2.resize(normal, dsize=(h, w),interpolation=cv2.INTER_CUBIC)   
  return normal, selected_images, selected_light_dirs
...
def benchmark(images, light_dirs, object_mask, normal_gt, algo, dtype=np.float32, debug=False):
  ...
  # normal_pred1 /= (np.linalg.norm(normal_pred1, ord=2, axis=2, keepdims=True) + 1e-6) #(128, 128, 3)
  # normal_pred1[~object_mask, :] = 0.0 #(128, 128, 3)
...
def benchmark_from_ids(data_loader, ids, algo, down_sample=2):
  ...
  selected_images_first = selected_images[-1:]
  selected_images_first = selected_images_first.transpose(0, 2, 1, 3)[:, ::-1, :, :]
  light_dirs_first = selected_light_dirs[-1:]
  images = [selected_images_first]
  lights = [light_dirs_first]
  
  if len(selected_images)>=2:
    selected_images_two = selected_images[-2:]
    selected_images_two = selected_images_two.transpose(0, 2, 1, 3)[:, ::-1, :, :]
    light_dirs_two = selected_light_dirs[-2:]
    images.append(selected_images_two)
    lights.append(light_dirs_two)
  

  return error, error_mean, N ,normal_gt,object_mask,images,lights

def benchmark_from_ids_unips_100(data_loader, ids, algo, down_sample=2):
      # ids = list(set(ids)) #[0]
  ids = list(ids) #[0]
  normal_gt, selected_images, selected_light_dirs = imload_downsample_from_ids_unips_100(data_loader, ids, down_sample) #(128, 128, 3) (1, 128, 128, 3) (1, 3)
  object_mask = minimum_filter(normal_gt[..., -1], size=3, mode="constant") > 0.0   #(128, 128)  Truth false#minimum_filter函数来对normal_gt的最后一个通道进行滤波处理,滤波后的结果与0.0进行比较，得到一个二值化的掩码（object_mask），其中大于0.0的值被视为1，小于等于0.0的值被视为0
  h = w = 512
  import cv2
  
  object_mask = object_mask.astype(np.float32)
  object_mask = cv2.resize(object_mask, dsize=(h, w),interpolation=cv2.INTER_CUBIC)#(512, 512)
  object_mask = object_mask.astype(np.bool_)
  normal_gt = cv2.resize(normal_gt, dsize=(h, w),interpolation=cv2.INTER_CUBIC)
